 ...
 * Stores the product in memory at the location of the pointer out
 */
__global__ void matVec2(float* ACSR, int* IA, int* JA, int M, float* b, float* out) {


	__shared__ float t_sum[BLOCK_SIZE_MAT]; // thread sum
	int j;

	int t_id = threadIdx.x + blockDim.x * blockIdx.x; // thread id
	int warpsPerBlock = blockDim.x / warpSize ;
	int row = threadIdx.x / warpSize + blockIdx.x* warpsPerBlock; // one warp per row
	int t_warp = threadIdx.x & (warpSize-1); // thread number within a given warp
	...
	if (row < M){
		...
		for (j = IA[row] + t_warp; j < IA[row+1]; j += warpSize)
		{
			t_sum[threadIdx.x] += ACSR[j] * b[JA[j]];
		}
...
__global__ void matVec3(float* A, float* b, float* out) {
...
void solveCG_cuda(float* d_A, float* A_CSR, int* JA, int* IA, float* d_UD, int* d_JU, int* d_IU, float* d_LD, int* d_JL, int* d_IL, float* b, float* x, float* p, float* r, float* z, float* temp,
	...
	// matVec2<<< M  / BLOCK_SIZE_MAT * 32, BLOCK_SIZE_MAT>>>(A_CSR, IA, JA, M, x, temp);

	// cudaMemcpy(h_data1, b, M * sizeof(float), cudaMemcpyDeviceToHost);
	
	// for(int i=0;i<M;i++)
	// {
	// 	std::cout << h_data1[i] << " : " << h_data1[i] << std::endl;
	// }
	// vecMinVec<<<vec_grid_dim, vec_block_dim>>>(b, temp, r);

	int k = 0;
	long micro_begin_gpu = getMicrotime();
	while ((k < MAX_ITER) && (*h_r_norm > EPS)) {

	    // cudaMemcpy(h_data1, r, M * sizeof(float), cudaMemcpyDeviceToHost);
	    ...
	    // cudaMemcpy(h_data2, z, M * sizeof(float), cudaMemcpyDeviceToHost);
		
		// for(int i=0;i<M;i++)
		// {
		// 	std::cout << h_data2[i] << " : " << h_data2[i] << std::endl;
		// }
		// exit(0);

        if (k == 0)
        {
			memCopy<<<vec_grid_dim, vec_block_dim>>>(z, p);
			vecVec2<<<vec_grid_dim, vec_block_dim>>>(r, z, numerator);
			// cudaMemcpy(h_var, numerator,  sizeof(float), cudaMemcpyDeviceToHost);
			
			// std::cout << "h_var : " << *h_var << std::endl;
			// exit(0);
        }
        else
        {
			// beta_k = ...
			memCopy<<<1, 1>>>(numerator, denominator);
			vecVec2<<<vec_grid_dim, vec_block_dim>>>(r, z, numerator);
			divide<<<1, 1>>>(numerator, denominator, beta);

			// cudaMemcpy(h_var, denominator,  sizeof(float), cudaMemcpyDeviceToHost);
			// std::cout << "h_var : " << *h_var << std::endl;
			// exit(0);

			// p_{k+1} = ...
			scalarVec<<<vec_grid_dim, vec_block_dim>>>(beta, p, temp);
			vecPlusVec2<<<vec_grid_dim, vec_block_dim>>>(z, temp, p);
        }

		matVec2<<< M  / BLOCK_SIZE_MAT * 32, BLOCK_SIZE_MAT>>>(A_CSR, IA, JA, M, p, temp);
	    // cudaMemcpy(h_data2, temp, M * sizeof(float), cudaMemcpyDeviceToHost);
		// matVec3<<<mat_grid_dim, mat_block_dim>>>(d_A, p, temp);

		
		// for(int i=0;i<M;i++)
		// {
		// 	std::cout << h_data1[i] << " : " << h_data2[i] << std::endl;
		// }
		// exit(0);
		// alpha_k = ...
		vecVec2<<<vec_grid_dim, vec_block_dim>>>(p, temp, temp_scal); //temp_scalar is denominator for alpha
